function [x,stat] = Levenberg_Marquardt_yq(fun_rJ, x0, tau, varargin)
% Input:
% tau: it is used for initializing lambda
...
%     end
    ...
    converged = (norm(df,'inf') <= tol || norm(p,2) <= tol*(norm(x,2)+tol));
